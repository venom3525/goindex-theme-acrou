...
var authConfig = {
   ...
   * The Basic Auth of each disk can be set separately. Basic Auth protects all folder/subfolder paths under this disk by default
   * [Note] The file link is not protected by default, which can facilitate direct-chain download/external playback;
   ...
   * No Basic Auth disk is required, just keep user and pass empty at the same time. (It can be set directly without setting)
   * [Note] The search function will not be supported for the disk whose id is set as the subfolder id (other disks will not be affected).
   */
  roots: [
    {
      id: "",
  ...
  /**
   * The number displayed on each page of the search result page. [Recommended setting value is between 50 and 1000];
   * If the setting is greater than 1000, it will cause an error when requesting drive api;
  ...
  // Confirm that cors can be turned on
...
var themeOptions = {
  cdn: "https://cdn.jsdelivr.net/gh/venom3525/goindex-theme-acrou",
  ...
  render: {
    ...
    head_md: false,
    /**
     * Whether to render the README.md ifile
 ...
 * @param gd
 * @returns {Promise<Response>} [Note] If the item represented by the id received from the front desk is not in the target gd disk, the response will return an empty string to the front desk ""
 */
